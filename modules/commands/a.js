...
const pathApi = path.join(__dirname, '../../includes/databot/');
		...
		if (args.length > 0) {
			...
			if (subCommand === 'list') {
			 ...
			 // let fileName = 'test.json';
				...
				const filePath = path.join(__dirname, '../../includes/databot/', fileName);
...
module.exports.handleReply = async ({ api, handleReply, event }) => {
	...
	try {
		...
		const getPath = (fileName) => path.join(__dirname, '../../includes/databot/', `${fileName}.json`);

		const getFilePath = (fileName) => path.join(__dirname, '../../includes/databot/', `${fileName}.json`);
